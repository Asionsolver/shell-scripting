...
- [x] What is Shell? (Bash, Zsh, Ksh, etc.)
- [x] Shell vs. Terminal vs. Bash
- [x] Installing and Setting Up Bash (Linux/macOS/WSL for Windows)
- [x] Essential Configuration Files (`~/.bashrc`, `~/.bash_profile`, `~/.zshrc`)
- [x] Setting Up a DevOps-Friendly Shell Environment
  - [x] PS1 Prompt Customization
  - [x] Useful Aliases and Functions
  - [x] Environment Variables (`$PATH`, `$HOME`, `$USER`)

## 2. Basic Scripting Skills

- [x] Writing Your First Shell Script (`.sh` file, shebang `#!/bin/bash`)
- [x] Executing Scripts (`chmod +x script.sh`, `./script.sh`, `bash script.sh`)
- [x] Variables and Data Types (String, Integer, Arrays)
- [ ] Reading User Input (`read` command)
- [ ] Basic Operators (Arithmetic, Relational, Logical)
- [ ] Control Flow:
  - [x] Conditional Statements (`if-else`, `case`)
